...
import { getWorkshopById, updateWorkshopAttendance } from "@/services/workshops.service";
...
export default function WorkshopAttendancePage() {
    ...
    useEffect(() => {
        const fetchSession = async () => {
            if (sessionId) {
                setLoading(true);
                try {
                    const data = await getWorkshopById(sessionId);
                    setSession(data);
                    if (data) {
    ...
    const handleSaveChanges = async () => {
        setSaving(true);
        try {
            await updateWorkshopAttendance(sessionId, attendance);
            toast({
                title: "¡Éxito!",
    ...
    return (
        <>
            <PageHeader title={`Asistencia Taller: ${session.title}`} description={`Taller del ${formattedDate} a las ${session.startTime}hs.`}>
                    ...
                    <TabsContent value="register">
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>Seleccione el estado de cada bombero, instructor y ayudante asignado a este taller.</CardDescription>
                            </CardHeader>
                            <CardContent>
