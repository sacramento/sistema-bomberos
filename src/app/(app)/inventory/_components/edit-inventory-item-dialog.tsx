...
import { updateGeneralInventoryItem } from "@/services/general-inventory.service";
...
export default function EditInventoryItemDialog({ children, item, onItemUpdated }: { children: React.ReactNode, item: GeneralInventoryItem, onItemUpdated: () => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await updateGeneralInventoryItem(item.id, { codigo, nombre, tipo, caracteristicas, estado, condicion, cuartel, ubicacion });
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-xl max-h-[90vh] flex flex-col">
                <DialogHeader>
                    ...
                    <DialogDescription>Modifique los detalles del ítem.</DialogDescription>
                </DialogHeader>
                <form onSubmit={handleSubmit} className="flex-grow overflow-y-auto pr-4 space-y-4 py-4">
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2"><Label>Tipo</Label><Select value={tipo} onValueChange={(v) => setTipo(v as any)}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{itemTypes.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent></Select></div>
                        <div className="space-y-2"><Label>Cuartel/Comisión</Label><Select value={cuartel} onValueChange={(v) => setCuartel(v as any)}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{cuarteles.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div>
                        <div className="space-y-2"><Label>Ubicación</Label><Select value={ubicacion} onValueChange={(v) => setUbicacion(v as any)}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{ubicaciones.map(u => <SelectItem key={u} value={u}>{u}</SelectItem>)}</SelectContent></Select></div>
