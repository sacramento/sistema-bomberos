...
import { updateAspiranteSession } from "@/services/aspirantes-sessions.service";
...
export default function EditAspiranteClassDialog({ children, session, onClassUpdated }: { children: React.ReactNode; session: Session, onClassUpdated: () => void; }) {
  const [open, setOpen] = useState(false);
  ...
  const handleSubmit = async () => {
    ...
    // Al editar, sincronizamos con la lista actual de aspirantes activos para cumplir el requerimiento de "siempre todos"
    ...
    try {
        ...
        await updateAspiranteSession(session.id, updatedData, { id: 'admin', name: 'Admin', role: 'Master', roles: { asistencia: 'Administrador', aspirantes: 'Administrador', semanas: 'Administrador', movilidad: 'Administrador', materiales: 'Administrador', ayudantia: 'Administrador', roperia: 'Administrador', servicios: 'Administrador', cascada: 'Administrador' } });

        toast({
            title: "¡Éxito!",
            description: `La clase de aspirantes ha sido actualizada.`,
        });
       ...
       case 3:
            return (
                <div className="space-y-4 text-sm">
                    <h4 className="font-bold text-base">Confirmar cambios:</h4>
                    <div className="p-4 bg-muted/50 rounded-lg space-y-3">
                       ...
                       <div className="pt-2">
                           <p className="font-semibold text-primary">Se actualizará la lista para incluir a los aspirantes actuales ({aspirantes.length}):</p>
                           {aspirantes.length > 0 && (
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
        ...
        <DialogContent className="w-[95vw] max-w-xl rounded-md flex flex-col max-h-[90vh]">
             <DialogHeader>
                <DialogTitle className="font-headline">Editar Clase de Aspirantes</DialogTitle>
                <DialogDescription>
                  Paso {step} de {totalSteps} - Modifique los detalles.
                </DialogDescription>
