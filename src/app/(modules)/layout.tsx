...
export default function AppLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        if (currentTopLevelPath && currentNavItem && !currentNavItem.roles.includes(user.role)) {
           // For a 'Bombero', their assistance link is '/reports', not '/sessions'
           if (user.role === 'Bombero' && currentTopLevelPath === '/sessions') {
                router.push('/reports');
                return;
           }

           const firstAvailablePage = navItems.find(item => item.roles.includes(user.role) && item.href !== '/dashboard');
